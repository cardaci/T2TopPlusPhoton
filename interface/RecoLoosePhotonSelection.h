...
void RecoLoosePhotonSelection(LepInfoBranches LepInfo, PhotonInfoBranches PhotonInfo, 
    ...
    for(int nl=0;nl<PhotonInfo.Size;nl++){
        ...
        if(     
		  ...
		  (max(PhotonInfo.phoPFChIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],0)*RhoPU,(float)0.) < 1.5) &&
		  (max(PhotonInfo.phoPFNeuIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],1)*RhoPU,(float)0.)<1.0+0.04*PhotonInfo.Pt[nl])
		  && 
		  (max(PhotonInfo.phoPFPhoIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],2)*RhoPU,(float)0.)<0.7+0.005*PhotonInfo.Pt[nl])
                  )
                 ||
                 (// Endcap
		  ...
		  (max(PhotonInfo.phoPFChIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],0)*RhoPU,(float)0.) <1.2) &&
