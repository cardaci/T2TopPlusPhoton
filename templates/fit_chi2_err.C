...
//////////  count the number of points and the dof   
  ...
  for (int i=1;i<2000;i++){ //the Sig or Bkg histo has 2000 bins
    if( ( ( bkgTemplate->GetBinContent(i) ) != 0) || ( ( sigTemplate->GetBinContent(i) ) != 0 ) ){
...
//     htmp2->Draw();
  ...
  Double_t scale_background = background_display->Integral();
  background_display->Scale((1-fsig)/scale_background);
